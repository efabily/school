...
/**
 * Base class that represents a row from the 'sch_person' table.
 *
 * 
 *
...
abstract class BasePerson extends BaseObject  implements Persistent {
	...
	/**
	 * @var        array Attribute[] Collection to store aggregation of Attribute objects.
	 */
	protected $collAttributes;

	/**
	 * @var        Criteria The criteria used to select the current contents of collAttributes.
	 */
	private $lastAttributeCriteria = null;
	...
	public function setIdState($v)
	{
		...
		if ($this->id_state !== $v) {
			$this->id_state = $v;
			$this->modifiedColumns[] = PersonPeer::ID_STATE;
		}
	...
	public function setDeletedBy($v)
	{
		...
		if ($this->deleted_by !== $v) {
			$this->deleted_by = $v;
			$this->modifiedColumns[] = PersonPeer::DELETED_BY;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		if ($deep) {  // also de-associate any related objects?
			...
			$this->lastAttributeCriteria = null;
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PersonPeer::ID_STATE)) $criteria->add(PersonPeer::ID_STATE, $this->id_state);
		if ($this->isColumnModified(PersonPeer::DELETED_BY)) $criteria->add(PersonPeer::DELETED_BY, $this->deleted_by);
	...
	/**
	 * Clears out the collAttributes collection (array).
	 *
	...
	/**
	 * Initializes the collAttributes collection (array).
	 *
	...
	/**
	 * Gets an array of Attribute objects which contain a foreign key that references this object.
	 *
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Person has previously been saved, it will retrieve
	 * related Attributes from storage. If this Person is new, it will return
		...
		if ($this->collAttributes === null) {
			if ($this->isNew()) {
			   $this->collAttributes = array();
				...
				$criteria->add(AttributePeer::PERSON_ID, $this->id);
	...
	/**
	 * Returns the number of related Attribute objects.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      PropelPDO $con
	 * @return     int Count of related Attribute objects.
	 * @throws     PropelException
	 */
	public function countAttributes(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collAttributes === null) {
			if ($this->isNew()) {
				...
				$count = AttributePeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
				...
				if (!isset($this->lastAttributeCriteria) || !$this->lastAttributeCriteria->equals($criteria)) {
					...
					$count = count($this->collAttributes);
				}
			} else {
				$count = count($this->collAttributes);
			}
	...
	public function addAttribute(Attribute $l)
	{
		...
		if (!in_array($l, $this->collAttributes, true)) { // only add it if the **same** object is not already associated
			array_push($this->collAttributes, $l);
			$l->setPerson($this);
		}
	}
	...
	public function countStudents(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		...
		if ($this->collStudents === null) {
			if ($this->isNew()) {
				...
				$count = StudentPeer::doCount($criteria, false, $con);
			}
		} else {
			// criteria has no effect for a new object
			if (!$this->isNew()) {
	 ...
	 */
	public function getTutorsJoinTypeTutor($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(PersonPeer::DATABASE_NAME);
		}
		...
		if ($this->collTutors === null) {
			if ($this->isNew()) {
				...
				$this->collTutors = TutorPeer::doSelectJoinTypeTutor($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collAttributes) {
				foreach ((array) $this->collAttributes as $o) {
